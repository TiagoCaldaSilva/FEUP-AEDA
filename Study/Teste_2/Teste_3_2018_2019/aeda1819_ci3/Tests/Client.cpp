...
bool Client::operator<(const Client& c1) const {
    return stoll(get<3>(propertyListing)) <= stoll(get<3>(propertyListing));
    //return stoll(get<3>(visitedProperties[visitedProperties.size() - 1])) < stoll(get<3>(c1.visitedProperties[visitedProperties.size() - 1]));
}
